...
read -p "Actualizar (Basico) [1] - Actualizar e instalar los paquetes dentro del script [2] - [3] Cambios visuales e internos: " ANSWER
REBOOT=N
if [ "$ANSWER" == "2" ]
then
echo ""
read -p "Reiniciar maquina despues de instalar todo? [Y/N]" REBOOT
fi
...
read -p "Queres reiniciar el sistema ahora mismo? [Y/N]: " REBOOT
;;
